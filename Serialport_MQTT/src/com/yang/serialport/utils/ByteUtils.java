...
public class ByteUtils {
	...
	public static int[] hexStr2Byte(String hex) {//将string字符串转为int整型
//		ByteBuffer bf = ByteBuffer.allocate(hex.length());
//		StringBuffer ret = new StringBuffer (""); 
		int[] intArray= new int[10];
		int length=hex.length();
		if(length!=8)
			ShowUtils.errorMessage("指令长度没有达到要求");
		for (int i = 0; i < 4; i++) {
			String hexStr = hex.substring(i*2,i*2+2);
			//System.out.println("~"+hexStr+"~");
			intArray[i]=Integer.parseInt(hexStr, 16);
		}
		
	    //byteArray = hex.getBytes();
	    return intArray;
//		return bf.array();
	}
